...
# ---------------------------------------------------------------------
# Discrete-Exterior-Calculus Echo-State Network (DEC-ESN) on a 2-complex
# ---------------------------------------------------------------------
class DECESN:
    """
    Discrete-Exterior-Calculus Echo-State Network (DEC-ESN).
    ...
    def _step(self, u_t: np.ndarray):
        ...
        # Hodge decomposition (on previous y)
        self._ensure_y_decomp()
        Pg_exact = self._y_exact
        Pg_coex  = self._y_coex
        Pg_harm  = self._y_harm

        y_leaked = (Y_prev
                    - self.leak_ex * Pg_exact
                    - self.leak_co * Pg_coex
                    - self.leak_ha * Pg_harm)
        y_diffused = (y_leaked - self.heat1 * self._apply_L1(Y_prev)).astype(np.float32)
        Y_new = (y_diffused + self.lam1 * xi_y).astype(np.float32)
...
sph_res = DECESN(
        ...
        faces             = None,     # optional: list of oriented faces (i, j, k) with boundary i→j→k→i
        avg_degree        = 6,        # used only if edges is None
    
        # Hodge stars (diagonal, SPD); None → all ones
        star0_diag        = None,
        star1_diag        = None,
        star2_diag        = None,
        ...
        ridge_alpha       = 1e-6,
        use_poly          = True,
        feature_mode      = "full",   # or "node", "full" "node_harm"
        seed              = 42,
    )
